# The Current/Future Status of ZMQ in Epsilon

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Epsilon
...
matching supported by both Epsilon and ZMQ. The `format`, `context` and `event`
...
The Epsilon daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Epsilon daemon also ensures that
...
The Epsilon daemon will send a `txpool_add` pub exactly once for each
