...
# a main offline epsilond, does most of the tests
# a restricted RPC epsilond setup with RPC payment
# two local online epsilonds connected to each other
N_EPSILONDS = 5

# 4 wallets connected to the main offline epsilond
# 1 wallet connected to the first local online epsilond
# 1 offline wallet
N_WALLETS = 7
...
epsilond_base = [builddir + "/bin/epsilond", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "epsilond_p2p_port", "--rpc-bind-port", "epsilond_rpc_port", "--zmq-rpc-bind-port", "epsilond_zmq_port", "--zmq-pub", "epsilond_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "epsilond_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
epsilond_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/epsilon-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_EPSILONDS):
  command_lines.append([str(18180+i) if x == "epsilond_rpc_port" else str(18280+i) if x == "epsilond_p2p_port" else str(18380+i) if x == "epsilond_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "epsilond_zmq_pub" else builddir + "/functional-tests-directory/epsilond" + str(i) if x == "epsilond_data_dir" else x for x in epsilond_base])
  if i < len(epsilond_extra):
    command_lines[-1] += epsilond_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/epsilond' + str(i) + '.log', 'a+'))
