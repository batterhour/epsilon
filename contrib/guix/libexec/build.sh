...
# Copyright (c) 2022-2024 The Epsilon Project
...
# target, so we don't spend time building Epsilon binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/epsilon-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,epsilon-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Epsilon build for HOST will be
