...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/epsilond"
  then
    epsilond="$dir/epsilond"
    break
  fi
done
if test -z "$epsilond"
then
  echo "epsilond not found"
  exit 1
fi
echo "Found: $epsilond"

TORDIR="$DIR/epsilon-over-tor"
TORRC="$TORDIR/torrc"
...
"$epsilond" \
  ...
  --add-priority-node 2epsnode5itf65lz.onion:18083 \
  --detach
ready=0
for i in `seq 10`
do
  sleep 1
  status=$("$epsilond" status)
...
if test "$ready" = 0
then
  echo "Error starting epsilond"
  tail -n 400 "$HOME/.bitepsilon/bitepsilon.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Epsilon\ \'" | tail -n 20
  exit 1
fi
